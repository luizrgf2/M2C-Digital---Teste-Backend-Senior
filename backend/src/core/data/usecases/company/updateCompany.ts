...
export class UpdateCompanyUseCase {
    ...
    private async checkIfNameAlreadyExists(name: string, companyId: string): Promise<Either<ErrorBase, void>> {
    ...
    private async storeCompany(id: string, update: UpdateCompanyProps): Promise<Either<ErrorBase, CompanyEntity>> {
        const saveOrError = await this.companyRepository.update(id, update);
    ...
    async exec(input: UpdateCompanyUseCaseInput): Promise<Either<ErrorBase, UpdateCompanyUseCaseOutput>> {
        ...
        if (input.name) {
            const checkIfNameAlreadyExistsOrError = await this.checkIfNameAlreadyExists(input.name, companyEntity.id);
        ...
        const saveCompanyOrError = await this.storeCompany(companyEntity.id, toUpdate);
